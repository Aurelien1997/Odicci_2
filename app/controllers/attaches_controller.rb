class AttachesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @attach.save
        format.html { redirect_to attach_url(@attach), notice: "Attach was successfully created." }
  ...
  def update
    respond_to do |format|
      if @attach.update(attach_params)
        format.html { redirect_to attach_url(@attach), notice: "Attach was successfully updated." }
  ...
  private
    ...
    def attach_params
      params.require(:attach).permit(:title, :body, :image)
    end
end
